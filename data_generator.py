...
class DataGenerator(object):
    ...
    def generate_sinusoid_batch(self, train=True, input_idx=None):
        ...
        init_inputs = np.zeros([self.batch_size, self.num_samples_per_class, self.dim_input]) # KL: store inputs 
        
        for func in range(self.batch_size):
            init_inputs[func] = np.random.uniform(self.input_range[0], self.input_range[1], [self.num_samples_per_class, 1]) # KL: get random x-values for input 
        ...
        #return init_inputs, outputs, amp, phase # for running sinusoid or sigmoid training 
